...
// https://pomax.github.io/bezierinfo/images/chapters/explanation/39d33ea94e7527ed221a809ca6054174.svg

const Bézier = (n: number, t: number): number => {
  let Σ = 0
  for (let i = 0; i < n; ++i) {
    Σ += (fact(n) / (fact(i) * fact(n - i))) * (1 - t) ** (n - i) * t ** i
  }
  return Σ
}

export const BézierCurve = (n: number, t: number): Array<[number, number]> => {
  const points: Array<[number, number]> = []
  for (let i = 0; i < 1 << (2 * n); ++i) {
    points.push([Bézier(i, n), t])
  }
  return points
}
...
const OhBézier: FunctionalComponent = () => {
  ...
  const art: Artwork = artworkLibrary['oh-bezier']
  return (
    <section class={styles.canvas_frame}>
      ...
      <button class="btn btn-primary position-fixed">+</button>
      <WebGL2 init={init} onResize={onResize} draw={draw} />
    </section>
  )
}

export default OhBézier
